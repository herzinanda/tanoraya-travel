...
const TourInfoGrid = ({ info }: TourInfoGridProps) => {
  ...
  const activeInfo = selectedDeparture?.variant?.tour_facilities ?? info
  ...
  return (
    <div className="activities-box-wrap">
      ...
      */}
      {Array.from({ length: Math.ceil(activeInfo.length / 4) }, (_, rowIndex) => (
        <div key={rowIndex} className={`activities-box-area ${rowIndex === 0 ? '' : 'mb-0'}`}>
          {activeInfo.slice(rowIndex * 4, (rowIndex + 1) * 4).map((item, itemIndex) => (
